...
const PostContent: React.FC<{ post: postInterface }> = ({ post }) => {
  ...
  useEffect(() => {
    axios
      .get(`${apiEndpoints.user}/?email=${post.author}`)
  ...
  return (
    <div className="mx-2 md:mx-0">
      <Head>
        ...
        <meta
          name="description"
          content={post.content.substr(0, Math.min(20, post.content.length))}
        />
        <link rel="icon" href="/favicon.ico" />
        <meta
          property="og:image"
          content={post.images ? post.images[0] : authorInfo.image}
        />
      ...
      <div className="flex flex-row space-x-1">
        ...
        <div className="w-full lg:w-9/12">
          <div className="bg-[#121212] border-2 border-[#DC143C] rounded-md ">
            ...
            <div className="rounded-md items-center">
              <div className="text-2xl md:text-4xl font-bold text-[#DC143C] mx-4 my-3 cursor-pointer title">
                {post.title}
              </div>
            </div>
            ...
            <div className="my-4 text-md md:text-lg font-semibold text-gray-300 mx-4 mb-8">
              {post.content.split("\n").map((ps: string, index) => {
        ...
        <div className="hidden lg:inline lg:w-3/12 rounded-md">
          <div className="border-2 bg-[#121212] border-[#DC143C] rounded-md flex flex-col p-2 items-center">
              <img className="rounded-full h-16 w-16" src={authorInfo.image} alt="author dp" />
              <h1 className="text-white font-bold text-lg">{authorInfo.name}</h1>
              <div className="my-2 flex text-gray-400 space-x-1">
                <p>17 posts</p>
                <p>/</p>
                <p>350+ reactions</p>
              </div>
              <button className="my-2 py-1 w-full bg-[#DC143C] rounded-md font-semibold text-white">Subscribe</button>
          </div>
        </div>
      </div>
    </div>
